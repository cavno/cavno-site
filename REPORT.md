...
- 症状：首页/栏目页正常，进入旧页（如 /life/driving/beginner-driving-guide/）后二级下拉菜单下半部被页面内容盖住。
...
## 2026-08-06 · 书籍页统一「目录—大纲」侧栏
- 现状普查（11 本）：已具备侧栏大纲 3 本（债务危机 236px 分组+高亮、历代经济变革得失 196px、债-5000年债务史 210px 两级）；其余为顶部吸顶横条或一次性平铺目录。
- 改造 7 本 → 对齐《债务危机》形态：中国国家治理的制度逻辑(13章/26节)、以利为利(13/42)、八次危机(9/6)、制内市场(10/22)、置身事内(6/8)、形式法则(7/24)、复杂(6/18)。
- 实现（`scripts/add-book-outline.mjs`，幂等、支持 --dry）：把 .lp 原有子节点整体包入 `.bk-main`，前置 `.bk-toc`，同置于 `.bk-shell` 弹性容器——**不改动原页任何既有结构与样式**；追加作用域 CSS（复用各页调色板变量并带兜底色）与 IntersectionObserver 滚动监听（章级高亮 + 自动展开其子目 + 侧栏自动滚动跟随 + 节级高亮）。
- 两级大纲：一级取 section[id] 的 h2；二级取其内 h3（无 id 者按 `<секid>-hN` 自动补 id，仅加属性不动内容）；《置身事内》的章级为 `div.chap`，改取其 id 与标题。锚点完整性已校验（如《复杂》18 个二级锚点与页内 h3 id 一一对应，缺失 0）。
- 原有页内导航加 `.bk-hidewide`：宽屏(>1080px)交给侧栏、窄屏仍由它承担，无功能损失亦无重复导航。**例外**：《复杂》的 `nav.spine` 每项带 `.desc` 说明文字，属正文内容，保留不隐藏。
- 未改动：《圣经》为标签页式工具（5 个分类面板、非线性长文），侧栏大纲不适用；已有侧栏的 3 本原样保留（脚本检测到 `bk-toc` 即跳过）。
- 验证：命名审计通过、构建通过、7 本产物侧栏与 CSS 均已注入，3 本未改动页 `bk-toc` 计数为 0。

## 2026-08-06 · 西方哲学史考察十讲：阅读版式重写
- 病根：全页正文以宋体族（Songti SC / STSong / SimSun）承担，16px/1.9。宋体横细竖粗，小字号在屏幕上发虚；且 Noto Serif SC 走 Google Fonts、大陆取不到（增量五已改非阻塞），回落即本机宋体——正是"字体不美观、可读性差"的成因。
- 排版主张（按字号而非喜好切分字族）：阅读级(<20px)→系统无衬线（PingFang SC / HarmonyOS Sans / MiSans / 微软雅黑，本机自带、字面锐利）；标题级(≥24px)→宋体族（大字号下笔锋才显气度，回落也好看）；书志级（讲次编号、日期、Era 标签、拉丁概念名）→等宽（它们是关于正文的信息，不是正文）。全页不依赖任何网络字体。
- 度量：正文 clamp(16→17.5px)/1.9，段落行距 2.05；版心 `min(100% - 36px, 46rem)`（CJK 约 40 字/行）；断点由原来的 2 个（700 / 420）增至 6 个（reduced-motion、1120、768、600、380）。
- 对比度修正：原 --ghost 等低对比标记色在羊皮纸上不足 3:1，已整体加深（body2/body3/muted/faint/ghost 五级重排）。
- 桌面（≥1120px）新增常驻侧栏：总览 + 六讲 + 四张总表，随路由高亮，宽屏不再只有一条居中窄栏且切换讲次必须回首页；此时顶栏横向导航自动隐去，避免重复。
- 移动端：顶栏导航改为可横滑、点按区 ≥40px，卡片内边距整体收紧，讲次翻页按钮换行堆叠，回到顶部按钮 44px。
- 新增阅读进度：顶栏底缘 2px 渐变细线（金→赭），长讲页尤其有用。
- 工程：样式表整体重写（20KB→27KB），旧表 251 个类**零遗漏**（脚本比对），新增 4 个为侧栏；数据体 D（六讲全文、脉络、谱系、张力、人物，占文件 85%）与全部渲染函数一字未动。

## 2026-08-07 · 付鹏大类资产投研框架页 + 原创示意图
- 新增 /investing/valuation/fu-peng-macro-asset-research-framework/：35 讲、11 万字文稿的内在逻辑重构，10 节 31 个二级条目，含书籍页同款侧栏大纲（两级 + 滚动高亮），末节为编者独立分析与评价。
- **未搬运课件原图（非遗漏，是判断）**：1308 张媒体全部是整页课件截图，带「华尔街见闻」水印与「大师课」品牌条，且大量为彭博/路透终端界面（第三方数据）。该页为公开页面（非 Working 的 Access 保护范围），搬运等同于重新发布付费课程。
- 替代方案：按课程论证结构**重画 7 张原创 SVG**——三段曲线（收入斜率 vs 金融化曲线，差值即贫富）、三级分工与债务落点、十步正反馈环、利率为唯一因的层级图、波动率二进制制度带、油价双杀与供给端囚徒困境、标普−东证的斜率剥离对冲。均为矢量、随容器缩放、复用页面调色板，移动端不失真；每图配 figcaption 说明其在论证中的位置。
- 验证：7 张 SVG 全部通过 XML 合法性校验；41 个目录锚点零失效；产物中无任何位图残留；命名审计与构建通过。
