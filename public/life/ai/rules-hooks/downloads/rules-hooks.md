...
# Claude Code Rules & Hooks 完整版

EP0019 Harness 配套资料：18 条 Rules、22 个 Hooks，脱敏后完整保留规则逻辑与结构。
...
**脱敏说明：**本文档为公开版本。所有 IP 地址、邮箱、微信号、SSH 密钥路径、VPN 架构细节等敏感信息已替换为占位符（如 `1.2.3.4`、`user@example.com`）。规则逻辑与结构完整保留，可直接参考和复用。
...
张拼拼AI实战教练 · [xntj.tv](https://xntj.tv/)

本文档为脱敏公开版，所有 IP 地址、邮箱、密钥路径、VPN 架构等敏感信息已替换为占位符。  
 规则和 Hooks 的逻辑结构完整保留，可直接参考和复用于你自己的 Claude Code 配置。

© 2026 张拼拼 (Max Pin)

---

来源：[https://cavno.org/life/ai/rules-hooks/](https://cavno.org/life/ai/rules-hooks/)
