...
用生活比喻讲清 122 个高频技术名词：它有什么用、什么时候会遇到，以及新手最常踩的坑。

[← 返回技能工具](http://127.0.0.1:5509/life/skills/)

**122** 个词条 **7** 大分类 每词 **4** 个维度 + 关联跳转 更新于 **2026-06-05**
...
vibecoder 网站做好要正式上线时,去阿里云/腾讯云/Cloudflare 等注册一个域名(.com 国内大约 75-95 元/年),绑到部署平台上,网址就从一串随机字符变成你自己的品牌名。
...
用户头像、商品图、上传的文档这类文件,放服务器硬盘上既占空间又难扩展,放对象存储则便宜、无限扩容、还能直接给出下载链接。AWS S3是最有名的对象存储,价格约每GB每月0.023美元。
...
[Redis]()[缓存]()[Serverless 无服务器]()

03

## 写代码与协作

动手做(或让 AI 做)东西时绕不开的开发概念
...
[REST API]()[JSON 数据格式]()[CI/CD 持续集成与部署]()
...
WebSocket 比普通 API 复杂:连接会断要会自动重连、用户多了服务器扛连接压力大、调试也更难。新手别一上来就自己【原文缺字】framework,能用现成的实时服务(如某些云服务)就别自己造。

[REST API]()[Webhook 回调]()[JSON 数据格式]()
...
[JSON 数据格式]()[技术栈]()[REST API]()

04

## AI 与大模型

vibecoder 的核心生产力,和 AI 打交道的语言
...
由 Anthropic 在 2024 年 11 月推出、2025 年底捐给 Linux 基金会旗下基金会托管,生态新、服务质量参差。装第三方 MCP 等于给 AI 开了访问权限,来路不明的别乱装,小心数据泄露。

[Agent 智能体]()[API调用 vs 本地部署]()[大语言模型]()
...
[Embedding 嵌入向量]()[RAG 检索增强生成]()[API调用 vs 本地部署]()
...
[API调用 vs 本地部署]()[参数量]()[大语言模型]()

05

## 安全与鉴权

不懂这层最容易出事:密钥泄露、被刷爆、数据被偷
...
[授权]()[Token / JWT 令牌]()[哈希 Hash(密码存储)]()[OAuth 第三方登录]()
...
[哈希 Hash(密码存储)]()[Token / JWT 令牌]()[.env 文件 / 密钥泄露]()
...
[SQL注入]()[CSRF 跨站请求伪造]()[Token / JWT 令牌]()
...
美国标准费率约为每笔 2.9% + 0.30 美元(stripe.com/pricing),跨境卡还要加约 1.5%,小额交易那 0.30 美元占比很高。另外 Stripe 在中国大陆不能直接给本地商户收人民币,国内生意得用支付宝/微信。
...
开通要营业执照等资质,个人开发者门槛比 Stripe 高。接入文档比 Stripe 繁琐,签名、证书容易踩坑。费率通常约 0.6% 起(随行业和资质浮动),以官方商户后台为准。
...
免费额度有限,Resend 免费版约每月 3000 封、每天 100 封(resend.com/pricing),超了要付费。发营销邮件还要遵守反垃圾邮件法规,域名也要做验证配置,否则照样进垃圾箱。
...
这就是你现在干的事本身。用 Cursor、Lovable、Claude 这类工具,靠对话一步步把脑子里的产品做出来,就是 vibe coding。它在 2025 年被柯林斯词典选为年度词汇,2026 年已是主流做法。
...
[Vibe Coding 氛围编程]()[代码编辑器 /]()[大语言模型]()[技术选型]()
...
[前端]()[HTML / CSS / JavaScript]()[开发者工具 / F12 控制台]()
...
[开源]()[库]()[GitHub / 代码仓库]()[技术选型]()

**怎么用这本词典** · 遇到看不懂的词先搜一下;每个词条底部的灰色标签是**关联词**,点一下直接跳过去,顺藤摸瓜建立全貌。  
 内容由多智能体并发调研 + 人工校订生成,面向零基础 vibecoder,力求准确好懂;价格/服务等时效信息以官方为准。

© 2026 张拼拼 (Max Pin) · zpplife@gmail.com · 保留所有权利

导出说明：原网页含 1 处已损坏的字符，本文件以“【原文缺字】”标明，未推测或改写缺失文字。

---

来源：[https://cavno.org/life/skills/tech-glossary/](https://cavno.org/life/skills/tech-glossary/)
